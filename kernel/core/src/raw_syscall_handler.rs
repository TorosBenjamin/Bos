...
use crate::syscall_handlers::{sys_channel_close, sys_channel_create, sys_channel_recv, sys_channel_send, sys_create_shared_buf, sys_debug_log, sys_destroy_shared_buf, sys_exit, sys_get_bounding_box, sys_get_display_info, sys_get_module, sys_get_time_ns, sys_lookup_service, sys_map_shared_buf, sys_mmap, sys_munmap, sys_mprotect, sys_mremap, sys_read_key, sys_try_read_key, sys_read_mouse, sys_register_service, sys_set_exit_channel, sys_set_fault_ep, sys_set_priority, sys_shutdown, sys_sleep_ms, sys_spawn, sys_thread_create, sys_transfer_display, sys_wait_task_ready, sys_waitpid, sys_yield, sys_try_channel_recv, sys_try_channel_send, sys_wait_for_event, sys_pci_config_read, sys_pci_config_write, sys_map_pci_bar, sys_alloc_dma, sys_ioport_read, sys_ioport_write, sys_get_ticks, sys_get_task_cpu_ticks, sys_pipe, sys_handle_read, sys_handle_write, sys_handle_close, sys_handle_dup, sys_handle_dup2, sys_spawn_with_handles, sys_handle_from_channel, sys_handle_open_keyboard, sys_handle_channel_create, sys_handle_wait, sys_handle_register_service};
use crate::task::task::{
...
pub fn init() {
    ...
    SYSCALL_TABLE.call_once(|| {
        ...
        table[SysCallNumber::ThreadCreate as usize] = Some(sys_thread_create);
        table[SysCallNumber::SetExitChannel as usize] = Some(sys_set_exit_channel);
        table[SysCallNumber::TryReadKey as usize] = Some(sys_try_read_key);
        table[SysCallNumber::TryChannelRecv as usize] = Some(sys_try_channel_recv);
        ...
        table[SysCallNumber::GetTimeNs as usize] = Some(sys_get_time_ns);
        table[SysCallNumber::Mprotect as usize] = Some(sys_mprotect);
        table[SysCallNumber::Mremap as usize] = Some(sys_mremap);
        table[SysCallNumber::SetFaultEp as usize] = Some(sys_set_fault_ep);
        table[SysCallNumber::WaitTaskReady as usize] = Some(sys_wait_task_ready);
        table[SysCallNumber::PciConfigRead as usize] = Some(sys_pci_config_read);
        table[SysCallNumber::PciConfigWrite as usize] = Some(sys_pci_config_write);
        table[SysCallNumber::MapPciBar as usize] = Some(sys_map_pci_bar);
        table[SysCallNumber::AllocDma as usize] = Some(sys_alloc_dma);
        table[SysCallNumber::IoPortRead as usize] = Some(sys_ioport_read);
        table[SysCallNumber::IoPortWrite as usize] = Some(sys_ioport_write);
        table[SysCallNumber::GetTicks as usize] = Some(sys_get_ticks);
        table[SysCallNumber::GetTaskCpuTicks as usize] = Some(sys_get_task_cpu_ticks);
        table[SysCallNumber::Pipe as usize] = Some(sys_pipe);
        table[SysCallNumber::HandleRead as usize] = Some(sys_handle_read);
        table[SysCallNumber::HandleWrite as usize] = Some(sys_handle_write);
        table[SysCallNumber::HandleClose as usize] = Some(sys_handle_close);
        table[SysCallNumber::HandleDup as usize] = Some(sys_handle_dup);
        table[SysCallNumber::HandleDup2 as usize] = Some(sys_handle_dup2);
        table[SysCallNumber::SpawnWithHandles as usize] = Some(sys_spawn_with_handles);
        table[SysCallNumber::HandleFromChannel as usize] = Some(sys_handle_from_channel);
        table[SysCallNumber::HandleOpenKeyboard as usize] = Some(sys_handle_open_keyboard);
        table[SysCallNumber::HandleChannelCreate as usize] = Some(sys_handle_channel_create);
        table[SysCallNumber::HandleWait as usize] = Some(sys_handle_wait);
        table[SysCallNumber::HandleRegisterService as usize] = Some(sys_handle_register_service);
        table
    });
}
