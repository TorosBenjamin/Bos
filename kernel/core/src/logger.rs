...
impl Log for KernelLogger {
    ...
    fn log(&self, record: &log::Record) {
        // Disable interrupts while holding the serial lock. Without this,
        // the timer can preempt a kernel task (e.g. the ELF loader) mid-write,
        // and the next task on the same CPU that tries to log spins forever
        // on the serial lock held by the preempted task.
